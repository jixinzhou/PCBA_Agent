...
- D037：SPI VTE方向性判定固定为`VTE<95%`支持少锡印刷候选、`VTE>105%`支持多锡印刷候选、`95%≤VTE≤105%`保持不确定，且不得表述为已确认的唯一根因。
- D038：T16先交付由Agent FastAPI直接托管的轻量本地Web供用户体验，交互确认后再进行Docker统一部署与GitHub项目包装，不引入独立React工程。
- D039：本地Web以聊天窗口为主交互，最终回复由Qwen综合RAG证据、候选状态与Tool结论生成五段式中文诊断报告，精确参数由程序追加且不得宣称唯一根因。
- D040：首版回流优化固定复用当前输入的唯一测温点，不向用户暴露optimization_point选择；多点高级输入仅确定性使用首点。
- D041：T16 Web只保留聊天与图片附件，Qwen结构化理解意图和显式参数，Tool选择、缺失项判断与状态路由继续由程序确定性执行。
- D042：每个Web会话持久化消息和诊断快照，解释型追问必须复用现有证据而不重新执行诊断，新案例由显式语义或新图片开启。
- D043：附图且消息为空或明确询问缺陷类别时确定性直达AOI，不允许LLM改写为原因诊断；新图片必须隔离上一案例的工艺观测。
