...
![PCBA Quality Copilot 中文诊断报告与候选致因](docs/assets/pcba-agent-diagnosis-report.png)

报告不是检索片段的简单拼接：LLM结合RAG知识证据生成诊断结论、候选原因分析、证据依据、处置建议与限制说明；程序再附加当前样本的Tool预测值、验证状态和候选致因卡片，避免LLM改写精确工艺事实。

### 参数优化与复验

![PCBA Quality Copilot SPI参数优化与复验](docs/assets/pcba-agent-optimization-revalidation.png)

系统仅在预测支持异常后调用优化Tool，并使用推荐参数再次预测。图中少锡样本的VTE由 `94.4844%` 回到 `99.9999%`，推荐状态为 `accepted`；全过程保留初始值、推荐参数、训练域判断和候选致因状态。
