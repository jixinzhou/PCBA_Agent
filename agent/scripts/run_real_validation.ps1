...
try {
    $serviceProcess = Start-Process `
        ...
        -WindowStyle Hidden `
        -RedirectStandardOutput (Join-Path $storageRoot "reflow.integration.stdout.log") `
        -RedirectStandardError (Join-Path $storageRoot "reflow.integration.stderr.log") `
        -PassThru
    ...
    & $pcbAgentPython (Join-Path $workspaceRoot "agent\scripts\validate_real_integration.py")
    if ($LASTEXITCODE -ne 0) {
        throw "T13 real integration validation failed with code $LASTEXITCODE"
    }
}
finally {
